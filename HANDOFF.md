...
> *"Hi Claude, kapatid — fresh session. Please read `HANDOFF.md`, `MULTIPLY_INVARIANTS.md`, `GRACE_PATHWAY.md`, `MULTIPLY_PIPELINE_DIAGRAM.md`, and `BTLI1_LESSON_MAP.md` first. **Session 40 — Multi-tenant Phase 2 is LIVE end-to-end.** Custom **HS256 JWT (Path 1, signed with the project's LEGACY shared secret** — the current signing key is asymmetric ECC P-256 and not exportable; the legacy HS256 secret still verifies, so we sign our own token with it; **never revoke the legacy key**; Inv #180) via the **`auth-login` Edge Function (verify_jwt OFF**, bcrypt-verifies the PIN with `npm:bcryptjs@2.4.3`, signs with EF secret `JWT_SECRET`; Inv #181). The client stores the token in `sessionStorage.multiply_jwt` (C1) and **`getDB()` attaches `Authorization: Bearer`** when present (C2); **`multiply_shared.js` is now `?v=7` with 19 consumers** (lockstep, Inv #184). `church_id` **auto-stamps on insert** via the `set_church_id_from_jwt` trigger (45 tables, except `devotionals`+backups; Inv #182). The **first RLS canary is LIVE on `debrief_records`** (`migrations/015`) — isolation **PROVEN** (Agape token → `[]`, Rosehill → 5 rows; reads + writes confirmed, a new debrief saved + revisited; Inv #185). **The DB client MUST carry the JWT for RLS (Inv #183):** MLT/MMT/MD route through `getDB()`; any standalone page with its OWN `createClient` must inline-attach the JWT before its tables get RLS. **NEXT ARC = RLS rollout** table-by-table (verify JWT paths → backfill NULL→church → 4 policies `TO authenticated` on `auth_church_id()` → ENABLE → one-line DISABLE rollback; test via a browser/curl token, **NEVER the SQL editor**). Watch the assessment token-flow + `members`/login + SECURITY DEFINER views BEFORE their RLS. Key IDs: Rosehill `724f4826-…`, Agape `46d78009-…`, Agape Test member `4c79ce89-…` (isolation-smoke account). Also still pending: **MLT 'My Profile' tile** (branch `mlt-my-profile`) and **BTLI L12** (needs the UNLAD-L2 scan). When I ask 'what's next?', refresh from this HANDOFF's pending list (Inv #43). Session-start: read the deployed file before editing (Inv #56); `node --check` + browser-smoke every change (Inv #105); bump `?v=` across all 19 after any `multiply_shared.js` deploy; SQL is human-gated (you propose, I run it in Supabase). Upload the latest deployed files first."*
