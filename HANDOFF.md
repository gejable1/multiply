...
**Last updated:** June 5, 2026 (Session 32 — **three quick fixes**: (1) **MLT lesson attachments** now download `.pptx`/office files on first click instead of opening a blank in-app viewer (`28722e2`, Inv #161); (2) **MD batch roster** alphabetized — both the member list and the "Pick a member" dropdown (`aa48d90`, `611a79a`); (3) **MD DISCIPLES tab** now keeps **guests** (`is_external_user`) visible — they were vanishing because the view lumped guests with test fixtures (`bccdff6`, Inv #160 — guests are operational-visible; only stats exclude them). All `multiply_dashboard.html`/`lc_leader_tool.html`; no `multiply_shared.js` change → `?v=4`. **2 invariants added: #160–#161. Count now 161.** Paused mid-session: self-attest **past-date** correction (open the >7-day window so old leader-marked absences can be flipped) — see PENDING.)
...
> *"Hi Claude, kapatid — fresh session. Please read `HANDOFF.md`, `MULTIPLY_INVARIANTS.md`, `GRACE_PATHWAY.md`, `MULTIPLY_PIPELINE_DIAGRAM.md`, and `BTLI1_LESSON_MAP.md` first. **Session 32 was three quick fixes** (all `multiply_dashboard.html`/`lc_leader_tool.html`, no `multiply_shared.js` change): (1) **MLT lesson attachments** download `.pptx`/office files on first click instead of opening a blank iframe viewer — `_isDownloadAttachment`/`_downloadAttachment`, `28722e2`, Inv #161; (2) **MD batch roster alphabetized** — member list + "Pick a member" dropdown (`aa48d90`, `611a79a`); (3) **MD DISCIPLES tab keeps guests visible** — `_disciplesDirect`/`_disciplesTree`/`_getUnassignedMembers` hid guests by lumping them with test fixtures; now test-only, so invited pastor-friends (`is_external_user`) stay in the relationship views (`bccdff6`, **Inv #160 — guests are operational-visible; only stats exclude them**). **PAUSED:** self-attest **past-date** correction — let members flip OLD leader-marked absences by opening the >7-day window (`SELF_ATTEST_WINDOW_DAYS`, member_tool.html); needs the window-policy decision first (see PENDING). Prior: **S31** Usbong quiz visible to batch staff (Inv #159); **S30** Usbong blank-stem (`_qText` + migration 002, Inv #158); **S29** self-attest present-aware + tenancy Phase 1 + attendance_admin (#152–#157). **TOP OPEN ITEM: BTLI L12 — Understanding My Unique Design** (UNLAD-L2, Eph 2:10) — needs the **UNLAD-L2 scan**. Next major arc = **multi-tenant Phase 2**. When I ask 'what's next?', refresh from this HANDOFF's pending list (Inv #43). **Session-start reminder: upload the latest deployed file(s) before editing (Inv #56); bump `?v=` after any `multiply_shared.js` deploy."*
...
### **⏸ Self-attest PAST-date correction** (PAUSED S32 — needs the window-policy decision, then build)
- **Goal (Pastor):** let a member flip an **old** leader-marked absence to present, not just the latest Sunday/Wednesday. The flip logic is already date-agnostic; the only blocker is `SELF_ATTEST_WINDOW_DAYS = 7` (`member_tool.html:7282`, enforced at `:7800`, `:7858/:7893`, `:7941`) and the picker only spanning 14 days.
- **Decision needed (the 7-day honesty window):** (A) **exempt corrections** of an existing leader-marked absence (any age; leader still confirms via Pending Confirmations) while keeping a window for brand-new claims [recommended]; (B) **extend** the window to N days for everything; (C) **remove** it, relying on the leader Confirm/Dispute overlay as the guard.
- **UX:** add a native `<input type="date">` (max=today) to the picker, or list past occurrences (past Sundays/Wednesdays). Applies to all plain events via one code path. No `multiply_shared.js` change expected.
