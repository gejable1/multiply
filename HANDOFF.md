...
> *"Hi Claude, kapatid — fresh session. Please read `HANDOFF.md`, `MULTIPLY_INVARIANTS.md`, `GRACE_PATHWAY.md`, `MULTIPLY_PIPELINE_DIAGRAM.md`, and `BTLI1_LESSON_MAP.md` first. **Session 39 — assessment Close/return polish + the shared `multiply_close.js` module (Inv #177): standalone pages get ✕ Close from a sibling module with its own `?v=`; MD opens assessments in a new tab (window.open → window.close returns to the modal), MMT same-frame (Inv #178: pageshow one-shot restore lands back on Discover). Assessment font-slider standard = `document.body.style.zoom`, max-width pages only (Inv #179 vs #176). Devotional streak now needs a ≥10-word reflection (Inv #173). Deployed `multiply_shared.js` = `?v=6` (older `?v=4` mentions are historical).** ✅ **The SVI EF 150s blocker is RESOLVED** — the `svi-ef-bulk-prefetch` refactor shipped + the Pastor redeployed `compute-svi-weekly`; the full 231-member compute now runs in **seconds**, reviewed clean (synchronous `computeMemberSnapshot`, zero per-member DB calls, Inv #170), dry-run parity verified (expected, no drift), and **LC-meeting scoring (Inv #171) is LIVE** (May Ann now < 70.67). Future recomputes still dry-run first (`?dry_run=true&member_id=…`, Inv #112). **Embed-timing rule (Inv #169):** in the shell iframe the session arrives async — never read `LEADER`/`isPastor()` synchronously at script-load; re-sync LEADER + re-run boot IIFEs inside `MultiplyShared.embedSessionReady().then()`. Also pending: **MLT 'My Profile' tile** (branch `mlt-my-profile`) and **BTLI L12** (needs the UNLAD-L2 scan). When I ask 'what's next?', refresh from this HANDOFF's pending list (Inv #43). Session-start: read the deployed file before editing (Inv #56); bump `?v=` after any `multiply_shared.js` deploy; upload the latest deployed files first. — Prior **Session 36 — `lc_group` decommission (Inv #166) Tiers 1–4 + Prayer Wave 2 + devotional-nag fix.** Canonical grouping is `discipler_id`/`facilitator_id`; the `lc_group` family is decorative (display-only). Frontend sweep is DONE (attendance reports, announcements, absence-notices, transfer-state, MMT LC-family all key on ids now; **Inv #167** three-state heartbeat, **Inv #168** devo-nag existence-gate). **lc_group decommission is COMPLETE (S37):** `migrations/004` (promote-by-id) + `005` (drop dormant indexes) RUN; `006` removed (the 3 views never existed). **Inv #166 strengthened:** ALL denormalized text mirrors (`discipler_name`/`member_name`/lc_group family/…) are decorative — resolve `id` → current record. Optional only: `migrations/007` (refresh stale `discipler_name`, human-gated) + Tier 4C `index.ts` svi passenger (Edge redeploy). Prayer: **Wave 2 shipped**, **W3/W4 HELD until your go**; cross-member writes need `SECURITY DEFINER` RPCs at Phase-2 RLS. **TOP OPEN ITEM: BTLI L12 — Understanding My Unique Design** (UNLAD-L2, Eph 2:10) — needs the UNLAD-L2 scan. When I ask 'what's next?', refresh from this HANDOFF's pending list (Inv #43). Session-start: read the deployed file before editing (Inv #56); bump `?v=` after any `multiply_shared.js` deploy. — Prior **Session 35 — Prayer / Panalangin Wave 1** (MMT-only): `migrations/003_prayer_wave1.sql` (run + verified, `all_ok:true`) created 4 per-church tables (`prayer_requests` / `prayer_list_items` / `prayer_list_opens` / `intercessions`); `member_tool.html` got the Home Prayer CTA, compose (4 audiences `lcg`/`individuals`/`all`/`discipler_pastor`), kind-scoped feed, save-to-list, My Prayer List, mark-answered — bilingual, **no points UI**, kind-bubbled per Inv #164 (helpers local to the prayer block; Phase B centralizes `sameKind` in `multiply_shared.js`). **Inv #165**, no shared.js change → `?v=4`. **Waves W2/W3/W4 are HELD until your go.** Prior: **S34** kind-mirroring Phase A (`fc586a1`, **Inv #164** — a real viewer/Pastor now SEES guest+test members badged; stats never cross-count kinds). Before that: **Session 33 — guest/test containment + self-attest past-date + viewer exit** (no `multiply_shared.js` change): (1) members **created by a guest/test user inherit that flag** — MD Add Member (NEW only: `checkbox OR (isNew && creatorFlag)`) + MLT `saveNewMember` (reads creator's row from DB) — so guest-added members never contaminate Rosehill stats (`4c48e83`, **Inv #162**); forward-only (a one-time backfill for pre-existing guest-added members is offered, not done). (2) **self-attest past-date** — correcting an OLD leader-marked absence works at **any age** (Option A: dup-check before the window; brand-new claims stay 7-day-gated) + native `<input type=date>` picker (`b4b792f`, **Inv #163**, extends #152). (3) **profile_results_viewer** ✕ Close button → returns to the firing tab; tablet/PWA users were stuck (`58944e9`, applies **Inv #12**). Prior: **S32** MLT pptx-download + MD roster alphabetize + MD guests-visible (#160–#161); **S29–S31** self-attest present-aware, tenancy Phase 1, attendance_admin, Usbong blank-stem + staff-visibility (#152–#159). **TOP OPEN ITEM: BTLI L12 — Understanding My Unique Design** (UNLAD-L2, Eph 2:10) — needs the **UNLAD-L2 scan**. Next major arc = **multi-tenant Phase 2** (and fold member-creation into one `newMemberDefaults(creator)` that also sets `church_id`). When I ask 'what's next?', refresh from this HANDOFF's pending list (Inv #43). **Session-start reminder: upload the latest deployed file(s) before editing (Inv #56); bump `?v=` after any `multiply_shared.js` deploy."*
