...
### **🧹 lc_group decommission (Inv #166)** — Frontend DONE (Tiers 1–4) · only human-gated SQL/Edge left
Canonical grouping = `discipler_id`/`facilitator_id`; `lc_group`/`member_lc_group`/`event_lc_group` are decorative.
- **Tiers 1–4 ✅ (frontend):** attendance reports (`e23554f`, `bca7871`, Inv #167) · audit + `diag_lc_group_views.sql` (`ba8d647`, 3 views dormant) · announcements `lcl:` keys (`e889be1`) · absence-notice own-disciples (`dc15c8f`) · transfer-state + MMT LC-family (`0dfc641`).
- **Remaining = human-gated, Pastor to run/decide** (`migrations/`, proposed `c3d262d`):
  1. **`004_promote_pending_lc_transfers_by_id.sql`** (+rollback) — fixes the DB fn to promote by `facilitator_id` (it currently moves only text). Recommend running.
  2. **`005_drop_dormant_lc_group_indexes.sql`** (+rollback) — OPTIONAL; drops 6 unused indexes, full rollback.
  3. **`006_drop_dormant_lc_group_views.sql`** (+rollback) — OPTIONAL · ⛔ **blocked**: first run `diag_lc_group_views.sql` (confirm no DB deps), paste the captured `pg_get_viewdef` into `006_rollback.sql`, then drop.
  4. **Tier 4C (deferred):** `index.ts` writes `lc_group` into `svi_snapshots` (passenger, decorative — `v_svi_latest` keys on `member_id`). Switch → `discipler_id` only if full purity wanted; needs `supabase functions deploy compute-svi-weekly`.
