...
- **Tier 4 — absence-notice heads-up ✅** (`lc_leader_tool.html`, `dc15c8f`): the MLT heads-up feed now filters by the member's live `discipler_id` → LCL (own disciples only, Pastor included — no church-wide bleed), dropping the `member_lc_group` text filter. Was the Pastor's reported symptom (non-flock heads-up in his MLT). Send side (MMT modal) untouched.
- **Tier 4 (remaining):** residual decorative `lc_group` cleanup from the Tier-2 audit — transfer-state text comparisons `pending_lc_group !== lc_group` → `pending_facilitator_id` (`lc_leader_tool.html` ~1494/1999/2026/2384, `transfer_management.html` 514/794); `member_tool.html` 6189/6569 `.eq('lc_group')` hasGroupText branch → always `discipler_id`; `index.ts` svi_snapshots `lc_group` passenger column. **Plus** dropping the dormant SQL views/indexes (`v_member_attendance_rates`/`v_attendance_summary`/`v_consecutive_absences` + `idx_*_lc_group`) once [diag_lc_group_views.sql](diag_lc_group_views.sql) confirms zero live consumers (human-gated SQL). Fold the full tier tracker into the next `m.md` with Inv #166.
