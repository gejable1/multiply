...
Deployed files re-read before editing (Inv #56); reviewed post-ship via `raw.githubusercontent.com`. **Invariants #169–#172 added → count 172.** ⚠️ The EF compute is **still blocked** by the 150s limit (Inv #170) — the LC-meeting scoring is shipped but **inert until the bulk-prefetch refactor lands and the Pastor redeploys the EF**.
...
- **EF (reviewed clean; INERT until the bulk-prefetch refactor + redeploy):** member-load gained `discipler_id`; `lcKeyOf(m)` = L2+ keyed by own id else `discipler_id`; a `meetingsHeld` Map (`Map<lcKey, Set<date>>`) is precomputed once from the unfiltered member map × 8-week LC-Meeting **present** attendance, passed into `computeMemberSnapshot`.
...
### **⚠️ SVI Edge Function — 150s timeout → `svi-ef-bulk-prefetch` refactor (TOP BLOCKER)** (Inv #170)
- **Problem:** the full **231-member** compute of `compute-svi-weekly` (repo-root `index.ts`) blows Supabase's **150s wall-clock hard limit** — logs `⏱150,201ms→546` / `⏱152,105ms→504`; the test-panel '500 Cannot read … (error)' is the panel choking on the killed function. Cause = **~2,500 sequential per-member DB queries** (~10/member). **Result: no `svi_snapshots` written → the LC-meeting scoring (Inv #171) is INERT** (May Ann still 70.67, `service_lc_led` 'not counted').
- **Decision (Pastor):** **Option 1 — durable bulk-precompute** (over the offset-batching stopgap). 'I can wait.'
- **The refactor (`svi-ef-bulk-prefetch`, prompt delivered, NOT shipped):** prefetch every source **ONCE** — attendance 8w (Sunday+LC), devotional 4w, interventions 26w, `diagnostic_results` latest-per-member, prayer ×4 `count_rows`, prior `svi_snapshots` @ weekStart−28d (trend) — **paginate past the 1000-row cap**, build per-member maps, then convert each querying handler (`computeSqlQuery`, `computeDateRecency`, `computeCountRows`, trend) to read the prefetch **in-memory** while keeping aggregation/rate/score logic **byte-identical**. Leave `applyScoreRules`, weighting, the `service_lc_led` special-case, and the LC softening **untouched**. Verify **no `await supabase.from()` remains in the per-member loop** + a parity harness.
- **Pastor-side after ship:** dry-run 2–3 members on the CURRENT EF first (Test panel → Query Params `member_id=<id>` + `dry_run=true`); **redeploy** `index.ts` via Supabase dashboard (Edge Functions → `compute-svi-weekly` → delete code → paste → Deploy); dry-run the SAME members → must be **IDENTICAL** (Inv #112). Then a full Compute should finish in **seconds**, and the LC-meeting scoring finally takes effect.
